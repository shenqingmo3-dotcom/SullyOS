...
export function buildInteractionModePrompt(char: CharacterProfile, userName: string): string {
    ...
    const format = mode === 'online'
        ...
        : `线下使用见面预设：普通文字视为当面说出口的话。先写一个连续的第三人称场景叙述块，再写中文引号对白（“……”）。合并同一瞬间的动作和环境，不写直接内心独白；延续地点和距离，移动写出过程，不瞬移，距离不足不能突然触碰。`;
    return `
### 当前互动状态（唯一有效）
${currentScene}

${format}
社交平台、网页和 MCP 在两种状态下都可使用，工具调用不会改变状态。只有用户明确切换或证据清楚地建立“来到同一地点/分别后继续发消息”时才改变状态，不要猜。切换时在回复最前输出隐藏控制标记，系统会自动吞掉：
