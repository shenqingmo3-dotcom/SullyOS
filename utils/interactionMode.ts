...
export function buildInteractionModePrompt(char: CharacterProfile, userName: string): string {
    ...
    const format = mode === 'online'
        ...
        : `线下使用见面预设：普通文字视为当面说出口的话。所有叙述必须使用第三人称：动作、身体移动、环境互动和场景描写要用角色名、他/她或 ta 作主语，不能用“我”写叙述；对白内部可以正常使用“我”。每个叙述块必须以 "> " 开头并独占一行；每个对白块必须使用中文引号“……”并独占一行，动作与对白不得出现在同一行或同一个气泡。可以根据场景自然交替多个叙述块和对白块，不限制段数。合并同一瞬间的动作和环境，不写直接内心独白；延续地点和距离，移动写出过程，不瞬移，距离不足不能突然触碰。`;
    return `
### 当前互动状态（唯一有效）
${currentScene}
