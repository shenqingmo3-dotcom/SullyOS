...
6. **可用动作**:
   ...
   - **添加纪念日**: 如果你觉得今天是个值得纪念的日子（或者你们约定了某天），你可以**主动**将它添加到用户的日历中。单独起一行输出: \`[[ACTION:ADD_EVENT | 标题(Title) | YYYY-MM-DD | 可选的一两句备注]]\`。
        ...
        return {
            apiMessages: historySlice.map((m, index) => {
                ...
                if (m.type === 'interaction') content = `${timeStr} [系统: 用户戳了你一下]`;
                    ...
                    const readable = normalizeMessageContent(
                        m,
                        char?.name || '你',
                        userProfile?.name || '用户',
                    );
                    content = `${timeStr} ${readable}`;
                    const mediaUrls = normalizeWebpageMediaUrls(webpage.mediaUrls, webpage.image);
