...
const StoryOutput: React.FC<{ content: string; onChoose?: (text: string) => void; affinityInputs: StoryAffinityInput[]; regexScripts?: StoryTheaterPreset['document']['regexScripts'] }> = ({ content, onChoose, affinityInputs, regexScripts }) => {
    ...
    return <div className='space-y-6'>
        ...
        {blocks.map((block, index) => {
            ...
            if (block.kind === 'story') return <div key={index} className='story-ocean-prose'><JournalRichText text={block.text} fontSize={15} lineHeight='1.72' fontFamily='"Iowan Old Style", "Songti SC", "STSong", serif' opts={{ color: 'var(--story-card-ink)', boldColor: 'var(--story-card-ink)', headColor: 'var(--story-card-ink)', accent: 'var(--story-card-accent)', muted: 'var(--story-card-muted)' }} /></div>;
                    ...
                    {pageMessages.map(message => {
                        ...
                        return <article key={message.id} {...pressHandlersFor(message)}><div className='story-dialog-row story-dialog-row-character'>
                            <div className={`story-dialog-card story-dialog-character ${streamStatus === 'streaming' ? 'story-streaming' : ''}`}>
                                ...
                                <header className='story-paper-head'><span className='story-paper-cast'>{actors.slice(0, 2).map(actor => <img key={actor.id} src={actor.avatar} alt='' className='story-dialog-avatar' />)}</span><span className='story-paper-meta'><strong>{actors.map(actor => actor.name).join('、') || '场景'}</strong><span>潮汐剧场 · {streamStatus === 'streaming' ? '正在写下这一页' : '本轮故事正文'}</span></span><span className='story-paper-issue'>#{String(message.id).slice(-3).padStart(3, '0')}</span></header>
