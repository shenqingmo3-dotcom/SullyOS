...
docker run --rm \
  ...
  bash -lc 'mkdir -p .deploy-cache/bin && corepack enable --install-directory .deploy-cache/bin && export PATH="/workspace/.deploy-cache/bin:$PATH" && pnpm install --frozen-lockfile && pnpm build'
